...
def reply_status(code: int):
    """
    gets the status code that was sent by the server
    """
...
class MySocket:
    ...
    def __init__(self, uid=None, sock=None):
        ...
        # create unique uid
        if uid is None:
            self.uid = random.randrange(0, 2 ** 32)  # 4 bytes
        else:
            self.uid = uid
        print(f'\nClient id: {self.uid}')
    ...
    def header(self, operation=0, name_len=0, filename="") -> bytes:
        ...
        :param filename:
        :return: bytes object that represent the client header.
        """
        if operation == 0:
    ...
    def send_file(self, fd):
        """
        reads the file content in chunks of size 1024 and
    ...
    def recieve_file(self, fd, file_size: int):
        ...
        and write them to fd
        :param file_size: the size of the file we retrieve
    ...
    def backup_file(self, filename: str):
        ...
        fd.seek(0)  # move fd pointer to the begining of the file
